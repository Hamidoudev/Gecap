{{-- <form action="{{ route('emplois.update', $emploi->id) }}" method="POST" enctype="multipart/form-data">
 ...
 <div class="container mt-5">
    <strong>
        <h3 class="detail-info">Détail Emplois {{ Auth::guard('enseignant')->user()->nom }}</h3>
    </strong>

    <p>Classe : {{ $DetailEmploi->classe->libelle }}</p>
    <p>Cycle : {{ $DetailEmploi->cycle->libelle }}</p>

    <!-- Affichage de l'enseignant en haut si le cycle est 1 -->
    ...
    <!-- Affichage de l'enseignant en bas si le cycle n'est pas 1 -->
    {{-- @if ($DetailEmploi->cycle->id = 2)
        <p>Enseignant : {{ $DetailEmploi->enseignant_id }} </p>
    @endif --}}

    <footer>
        <div class="footer-button">
            <!-- Assurez-vous que vous passez l'identifiant correct pour le téléchargement -->
            <a class="btn btn-primary" target="_blank" href="{{url ('/pages/enseignant/emplois/pdf/'.$DetailEmploi->id)}}">Télécharger</a>
        </div>
    </footer>
</div>
